...
vVehMet = wVehList/MPH_2_KMPH*3600/1000*R_TIRE; % [mph]
aVehMet = aVehList/MPH_2_KMPH*3600/1000*R_TIRE; % [mph/s]
 
% select comfortable acceleration region
selAccel = 121:181; % -3mph-3mph, 165:deSamp:end, 101:201, 145:163 121:181
aVehComf = aVehMet(selAccel); 
aoComf = aoList(selAccel, :);
boComf = boList(selAccel, :);

% can use mesh(vVehMap, aVehMap, aoMap) to plot
[vVehMap, aVehMap] = meshgrid(vVehMet, aVehComf); % [mph], [mph/s]
aoMap = aoComf; %[metric]
boMap = boComf; %[metric]

% for fitting, need to reshape as vectors, convert to metric units
vVehFit = reshape(vVehMap*MPH_2_KMPH*1000/3600, [], 1);
aVehFit = reshape(aVehMap*MPH_2_KMPH*1000/3600, [], 1);
aoFit = reshape(aoMap, [], 1);
boFit = reshape(boMap, [], 1);

%% Fit ao bo using polynominals
aoFit = fit([vVehFit, aVehFit], aoFit, 'poly24');
boFit = fit([vVehFit, aVehFit], boFit, 'poly22');
aoCoeff = coeffvalues(aoFit);
boCoeff = coeffvalues(boFit);
...
fuelConsFcn = @(v, a, pBatt) aoFit(v, a)*pBatt + boFit(v, a);
...
aoEst = aoFit(vVehEst, aVehEst);
boEst = boFit(vVehEst, aVehEst);
...
vPre = vVehEst+2;
vVehCur = vVehEst;
stateCur = [10 vVehCur 0.6]; % state [d, v, SOC]'
